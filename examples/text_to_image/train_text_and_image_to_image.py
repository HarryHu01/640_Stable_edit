...
def parse_args():
    ...
    ############################################################
    parser.add_argument(
        "--input_image_column", type=str, default="image", help="This is the input column that will replace the noise."
    )
...
dataset_name_mapping = {
    ...
    ##"stable_edit/validation_data": ("output_image","input_image", "text"),
}
...
def main():
    ...
    ####So the first column is the output image, second column is input image, third columns is the caption
    #############################################################################
    if args.input_image_column is None:
        input_image_column = dataset_columns[1] if dataset_columns is not None else column_names[1]
    else:
        input_image_column = args.input_image_column
        if input_image_column not in column_names:
            raise ValueError(
                f"--image_column' value '{args.input_image_column}' needs to be one of: {', '.join(column_names)}"
            )
    ############################################################################
    ...
    def preprocess_train(examples):
        ...
        ##this does the captions
        examples["input_ids"] = tokenize_captions(examples)

        ######This is the input images converted to pixel values.
        input_images = [image.convert("RGB") for image in examples[input_image_column]]
        examples["input_pixel_values"] = [train_transforms(image) for image in input_images]
